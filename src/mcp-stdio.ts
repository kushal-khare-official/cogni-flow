...
import { workflowToToolName, extractWorkflowId } from "./lib/mcp-server/tool-name";
import { requestBodyToJsonSchema, type RequestBodyField } from "./lib/mcp-server/schema";
import { PLATFORM_TOOLS, isPlatformTool } from "./lib/mcp-server/platform-tools";
...
async function handleGetWorkflowSchema(params: Record<string, unknown>) {
  try {
    ...
    if (endNode?.data.responseMapping) outputSchema.responseMapping = endNode.data.responseMapping;
    if (endNode?.data.outputSchema) outputSchema.outputSchema = endNode.data.outputSchema;
...
async function handleGenerateWorkflow(params: Record<string, unknown>) {
  try {
    ...
    const systemPrompt =
      ...
      "Integrations have no operations. Each workflow step is a serviceTask with integrationId and stepConfig. Use an existing integration ID when one fits; otherwise leave integrationId null and populate newIntegration.\n\n" +
      integrationContext;
...
async function handlePlatformTool(name: string, params: Record<string, unknown>) {
  switch (name) {
    case "list_workflows": return handleListWorkflows(params);
    case "get_workflow_schema": return handleGetWorkflowSchema(params);
    case "get_execution_history": return handleGetExecutionHistory(params);
    case "validate_workflow": return handleValidateWorkflow(params);
    case "dry_run_workflow": return handleDryRunWorkflow(params);
    case "generate_workflow": return handleGenerateWorkflow(params);
    default: return textResult(`Unknown platform tool: ${name}`, true);
  }
}

// ── Dynamic workflow tool handler ───────────────────────────────────

async function handleWorkflowTool(toolName: string, params: Record<string, unknown>) {
  const idPrefix = extractWorkflowId(toolName);
  if (!idPrefix) return textResult("Invalid tool name", true);

  const workflows = await prisma.workflow.findMany({
    where: { status: { in: ["live", "shadow"] } },
  });
  const workflow = workflows.find((wf) => wf.id.startsWith(idPrefix));
  if (!workflow) return textResult(`Workflow not found for tool: ${toolName}`, true);
  ...
  if (!startNode) return textResult("Workflow has no start event", true);
  ...
  if (requestBodySchema && requestBodySchema.length > 0) {
    ...
    if (errors.length > 0) return textResult(`Validation failed: ${errors.join(", ")}`, true);
  }

  const run = await prisma.executionRun.create({
    data: { workflowId: workflow.id, trigger: "mcp", status: "running", input: JSON.stringify(input) },
  });
  ...
  try {
    ...
    if (result.error) return textResult({ error: result.error, output }, true);
...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (isPlatformTool(toolName)) return handlePlatformTool(toolName, params);
    return handleWorkflowTool(toolName, params);
  });

  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("CogniFlow MCP server running on stdio");
}
