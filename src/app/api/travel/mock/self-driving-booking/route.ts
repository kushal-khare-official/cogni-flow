...
export async function POST(request: NextRequest) {
  try {
    ...
    if (booking.type !== "selfDriving") {
      return NextResponse.json(
        { error: "self-driving-booking API only accepts selfDriving integration type" },
        { status: 400 },
      )
    }
    ...
    console.error("[travel/mock/self-driving-booking]", error)
    const message =
